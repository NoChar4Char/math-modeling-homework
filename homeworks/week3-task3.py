...
Y_0 = np.array([PI, 1]) # default: np.array([np.pi/4, 0]), which matches the actual for linearized.
...
nonlin_pendulum.rk4()
nonlin_pendulum.plot("red", False, False, "Runge-Kutta 4")
nonlin_pendulum.backward_euler_nonlin_pendulum()
nonlin_pendulum.plot("green", False, False, "Backward Euler")
plt.legend()
plt.show()